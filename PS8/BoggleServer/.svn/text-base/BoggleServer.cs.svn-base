...
namespace BC
{
    public class BoggleServer
    {
        ...
        public BoggleServer(int _gameLength, String dictionary_path)
        {
            ...
            // initialize player queue and cancelled players set
        ...
        private void NameReceived(string s, Exception e, object payload)
        {
            ...
            if (s.Substring(0, 4) == "PLAY")
            {
                ...
                PlayerQueue.Enqueue(new Player(s.Substring(5), payload));

                // lock when dequeueing players
                lock (LockSockets)
                {
                    // clean up the queue if it's small enough to matter
                    if ((PlayerQueue.Count > 0) && (PlayerQueue.Count < 10))
                        PlayerQueue.RemoveCancelledPlayers();
        ...
        private void MessageReceived(string s, Exception e, object payload)
        {
            ...
            lock (LockSockets)
            {
                ...
                Game currentGame;
                if (GameSet.TryGetValue(payload.GetHashCode(), out currentGame))
                {
                ...
                // if game is not associated with this player, they're still in the queue
                {
                    if (e is DisconnectedException)
                    {
                        // player has cancelled their play request
                        PlayerQueue.Cancel(((StringSocket)payload).GetHashCode());
                    }
                    else
                    {
                        // the input doesn't conform to any of the commands we recognize, or is invalid for
                        // unpaired players
                        ((StringSocket)payload).BeginSend("IGNORING", callback, payload);
                    }
                }
                ...
                {
                    // the player must be disconnected, so if a game is going on, raise a disconnected player event
                    if (currentGame != null)
                    {
                        DisconnectedPlayerEvent(this, new DisconnectedPlayerEventArgs(currentGame.GetPlayer(((StringSocket)payload).GetHashCode()).Opponent.Payload));
                    }
                    ...
                    {
                        // a game isn't going on, so remove this connection from the queue
                        PlayerQueue.Cancel(((StringSocket)payload).GetHashCode());
                    }
                }
            }
    ...
    /// </summary>
    public class PlayerQueue : Queue<Player>
    {
        HashSet<int> CancelledPlayers;

        public PlayerQueue()
            : base()
        {
            CancelledPlayers = new HashSet<int>();
        }

        /// <summary>
        /// Removes players who have disconnected before being paired
        /// </summary>
        public void RemoveCancelledPlayers()
        {
            Queue<Player> temp = new Queue<Player>();
            
            // dequeue into a temp queue, removing cancelled players in the process
            while (this.Count > 0)
                temp.Enqueue(this.Dequeue());

            // enqueue from the temp queue, filling this back up with non-cancelled players
            while (temp.Count > 0)
        ...
        /// </summary>
        /// <param name="p"></param>
        public new void Enqueue(Player p){
            // remove player from the cancelled players set if present
            CancelledPlayers.Remove(p.GetHashCode());
        ...
        /// </summary>
        /// <returns></returns>
        public new Player Dequeue()
        {
            // get the first element off the queue
            Player p = base.Dequeue();
            
            // continue getting elements off the queue until it is a non-cancelled player
            while ((base.Count > 0) && (CancelledPlayers.Contains(p.GetHashCode())))
            {
                // remove the player from the cancelled set, since their request has been cancelled
                CancelledPlayers.Remove(p.GetHashCode());
            ...
            // if the queue's size is zero, no need to hold any cancelled players
            if (base.Count == 0)
        ...
        {
            // add the player to the set of cancelled players
            CancelledPlayers.Add(hashcode);
        }
    }
